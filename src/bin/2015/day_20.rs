...
fn sieve_eratosthenes(n: usize) -> Vec<usize> {
    println!("Computing primes in the range 1..{}", n);
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("The first house to get {} presents is house #{}", number, house);

    let house = first_house_with_n_presents_finite(number, 50);
    println!("If each elf only visits 50 houses, the first house to get {} presents is house #{}", number, house);
