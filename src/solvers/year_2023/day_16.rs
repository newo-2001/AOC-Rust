...
impl Default for Beam {
    fn default() -> Self { Self(Point2D::zero(), CardinalDirection::East) }
}

impl Beam {
    fn forward(self, grid: &Grid<Tile>) -> Option<Self> {
        let Self(location, direction) = self;
        let new_pos = location.checked_add::<isize>(direction.direction_vector())?;
        let beam = Self(new_pos, direction);
        let t = beam;
        grid.get(new_pos).is_some().then_some(t)
    }

    fn energize(self, grid: &Grid<Tile>) -> usize {
        let mut filter = once(self).collect_vec().filter_duplicates();
        filter.recursive_iter(|Self(location, direction)| {
            type Dir = CardinalDirection;

            grid.get(location).map_or(IterState::Leaf, |tile| {
                let beams = match (tile, direction) {
                    ...
                    .filter_map(|direction| Self(location, direction).forward(grid))
                    .collect_vec();
        ...
        filter.seen
            .into_iter()
            .unique_by(|&Self(location, _)| location)
            .count()
    }
}
